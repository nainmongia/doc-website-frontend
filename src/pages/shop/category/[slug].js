...
export default function () {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      if (slug.length !== 0) {
        const url = `${baseUrl}/api/search/category/wise/products?page=1&size=12&search=${slug}`;
  ...
  async function PageData(page) {
    try {
      setLoading(true);
      const url = `${baseUrl}/api/search/category/wise/products?page=${page}&size=12&search=${slug}`;
      const res = await axios.get(url, { withCredentials: true });
      console.log(res);
      setCount(res.data?.count);
      setProductData(res.data?.result);
  ...
  return (
    ...
    <>
      <LayoutOne title="Shop Fullwidth Left Sidebar">
        <Breadcrumb title="Shop Products By Category">
